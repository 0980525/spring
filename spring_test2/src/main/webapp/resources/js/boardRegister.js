...
const maxSize = 104*1024*20;//파일 최대 크기
...
document.addEventListener('change',(e)=>{
    ...
    if(e.target.id == 'files'){ //파일에 대한 변화가 생겼다면 
        //input file element 에 
        const fileObj = document.getElementById('files').files;
        console.log(fileObj);

        //한번 disabled 되면 혼자 풀어질 수 없음. 버튼 원래상태로 복구
        ...
        //여러 파일에 대한 검증을 모두 통과하기 위해서 *로 각 파일마다 통과 여부확인
        let isOk = 1; //전체파일의 검증 결과
        ...
        if(isOk == 0){
            //하나의 파일이라도 검증에 통과하지 못한다면 버튼 활성화
